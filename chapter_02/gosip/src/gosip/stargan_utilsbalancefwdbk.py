...
class WeightedZIGNLLLoss(nn.Module):
    ...
    mixing point‑mass at zero with Gaussian density at zero.
    """
    ...
    def forward(
        ...
        # 1) clamp pi and compute one_minus_pi
        pi = pi.clamp(self.eps, 1 - self.eps)
        one_minus_pi = (1 - pi).clamp(min=self.eps)

        # 2) variance term
        var = sigma.pow(2).add(self.eps)
        ...
        # 4) Gaussian log-density at zero
        log_gauss_zero = -0.5 * (
            torch.log(2 * math.pi * var)
            + mu.pow(2).div(var)
        )
        # continuous density at zero
        dens_zero = torch.exp(log_gauss_zero).clamp(min=self.eps)

        # 5) zero vs nonzero mask (exact zero) — for floats, consider small threshold if needed
        zero_mask = (x == 0).float()
        nonzero_mask = 1.0 - zero_mask

        # 6) zero-branch log-prob: logsumexp of [log pi, log(1-pi)+log_gauss_zero]
        mix = torch.stack([
            torch.log(pi),
            torch.log(one_minus_pi) + log_gauss_zero
        ], dim=0)
        log_prob_zero = torch.logsumexp(mix, dim=0)

        # 7) nonzero-branch log-prob: log(1-pi) + log Gaussian
        log_prob_nonzero = torch.log(one_minus_pi) + log_gauss

        # 8) select appropriate branch
        log_prob = zero_mask * log_prob_zero + nonzero_mask * log_prob_nonzero

        # 9) negative log-likelihood and weighting
        nll = -log_prob
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                for data, labels, weights, global_weights in val_loader:
                    ...
                    val_loss += ( self.lambda_rec * (loss_gsg + loss_sgs) / 2.0 )
